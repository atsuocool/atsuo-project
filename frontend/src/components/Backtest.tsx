...
export default function Backtest() {
  ...
  const inp: React.CSSProperties = { background: '#1e293b', color: '#e2e8f0', border: '1px solid #334155', borderRadius: 6, padding: '8px 12px', width: '100%' };
    ...
    <div style={{ maxWidth: 900 }}>
      ...
      <div style={{ background: '#1e293b', borderRadius: 8, padding: 20, marginBottom: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div><label style={{ fontSize: 12, color: '#94a3b8' }}>シンボル</label><input value={form.symbol} onChange={e => setForm({ ...form, symbol: e.target.value })} style={inp} /></div>
          <div>
            ...
            <select value={form.strategy} onChange={e => setForm({ ...form, strategy: e.target.value })} style={inp}>
              <option value="ma_cross">MA クロス</option>
              <option value="rsi_bb">RSI + ボリンジャーバンド</option>
              <option value="macd">MACD</option>
            </select>
          </div>
          <div><label style={{ fontSize: 12, color: '#94a3b8' }}>初期資金 (USD)</label><input type="number" value={form.initial_capital} onChange={e => setForm({ ...form, initial_capital: Number(e.target.value) })} style={inp} /></div>
          <div><label style={{ fontSize: 12, color: '#94a3b8' }}>開始日</label><input type="date" value={form.start_date} onChange={e => setForm({ ...form, start_date: e.target.value })} style={inp} /></div>
          <div><label style={{ fontSize: 12, color: '#94a3b8' }}>終了日</label><input type="date" value={form.end_date} onChange={e => setForm({ ...form, end_date: e.target.value })} style={inp} /></div>
        </div>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
            {[
              ...
              { label: 'シャープレシオ', value: String(result.sharpe_ratio) },
